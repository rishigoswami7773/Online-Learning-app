...
class EnrollmentModel {
  ...
  factory EnrollmentModel.fromMap(
    ...
    // Support both old (enrolled_courses) and new (enrollments) field names
    final userId =
        (data['userId'] ?? data['studentId'] ?? '').toString();
    // progressPercent is 0-100 integer; progress is 0.0-1.0 double
    double progress;
    if (data['progressPercent'] != null) {
      progress = (_safeDouble(data['progressPercent'], fallback: 0.0) / 100.0)
          .clamp(0.0, 1.0);
    } else {
      progress = _safeDouble(data['progress'], fallback: 0.0);
    }
    final completedLessons = _safeInt(
      data['completedLessons'] ?? data['lessonsCompleted'],
      fallback: 0,
    );
    final lastViewedAt =
        _safeDate(data['lastViewedAt'] ?? data['lastAccessedAt']);
    return EnrollmentModel(
      ...
      courseThumbnail: (data['courseThumbnail'] ?? data['thumbnailUrl'] ?? '').toString(),
      progress: progress,
