...
class DashboardData {
  ...
  final List<LegacyCourseModel> allCourses;
  final List<EnrollmentModel> continueLearning;
  final List<LegacyCourseModel> featuredCourses;
  final List<LegacyCourseModel> trendingCourses;
  final List<LegacyCourseModel> recommendedCourses;
...
class DashboardController {
  ...
  Future<DashboardData> loadDashboardData({bool forceRefresh = false}) async {
    ...
    try {
      // Run parallel tasks with timeouts so the Future always completes
      final futures = [
        _fetchStudentName(),
        _courseController.fetchCourses(forceRefresh: forceRefresh),
        _enrollmentController.fetchEnrollments(forceRefresh: forceRefresh),
        _fetchUserStats(),
      ].map((f) => (f as Future).timeout(const Duration(seconds: 12))).toList();

      final results = await Future.wait(futures);
      debugPrint('Dashboard load fetched remote data');

      final studentName = results[0] as String;
      final allCourses = results[1] as List<LegacyCourseModel>;
      ...
      // Re-throw so callers (FutureBuilder) receive the error and can show UI
      rethrow;
    }
  }
  ...
  List<LegacyCourseModel> _getRecommendedCourses(
    List<LegacyCourseModel> allCourses,
