...
class _EditProfileScreenState extends State<EditProfileScreen> {
  ...
  List<String> _availableCourses = <String>[
  ...
  Future<void> _loadCourses() async {
    try {
      ...
      if (mounted && titles.isNotEmpty) {
        setState(() => _availableCourses = titles);
      }
    } catch (_) {}
  }

  Future<void> _pickAndUploadImageFromFiles() async {
    if (_isUploadingImage || _isPickingImage) {
      return;
    }
    ...
    try {
      ...
      if (currentUser != null) {
        authController.currentUser.value = AppUser(
          ...
          role: currentUser.role,
          profileImage: _profileImageUrl.isNotEmpty
              ? _profileImageUrl
                      ...
                      child: Stack(
                        children: [
                          ...
                          Positioned(
                            ...
                            bottom: 0,
                            child: InkWell(
                              onTap: _isUploadingImage || _isPickingImage
                                  ? null
                                  : _pickAndUploadImageFromFiles,
                              child: CircleAvatar(
                                radius: 16,
                                child: _isUploadingImage || _isPickingImage
                                    ? const SizedBox(
                                        width: 12,
                                        height: 12,
